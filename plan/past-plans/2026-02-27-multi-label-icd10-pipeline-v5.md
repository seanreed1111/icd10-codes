# Multi-Label ICD-10 Retrieval Pipeline v5 — Implementation Plan
...
- [Phase 1: Create pipeline_v5.py](#phase-1-create-pipeline_v5py)
...
Implement a multi-label ICD-10 retrieval pipeline (`src/pipeline_v5.py`) that processes clinical notes and predicts sets of ICD-10-CM codes. The pipeline uses a sliding-window chunking approach (Option E from v2) to handle long clinical notes.
...
**Key changes from v4 (based on v4 review):**

1. **Performance fix (high priority):** The `__main__` block now builds `TfidfRetriever` exactly **once** and reuses it for the baseline config and all 27 sweep configs. In v4, `build_pipeline()` was called 28 times (1 baseline + 27 sweep), each rebuilding the TF-IDF index over 74,719 entries. Now `PipelineConfig` is constructed directly in the sweep loop with the shared retriever — reducing index builds from 28 to 1. Estimated runtime drops from 20–40 minutes to 3–8 minutes.
2. **Unused import fix:** Removed `ICD10Code` from `from knowledge_base import ...`. `ICD10Code` was imported in v4 but never used in any function signature or body, which would cause `ruff check` to fail with `F401`.
3. **`NotImplementedType` under `TYPE_CHECKING` guard:** With `from __future__ import annotations`, all annotations are strings at runtime and `NotImplementedType` is never needed at import time. Moving it under `TYPE_CHECKING` avoids an unnecessary runtime import and follows Python best practice for typing-only imports.
4. **Documented `(empty, empty) → (1,1,1)` edge case:** Added inline comment in `precision_recall_f1` explaining that both-empty is treated as vacuous truth (intentional).
5. **Documented ALL_CAPS naming convention:** Added note in module docstring explaining `IGNORE_THREE_CHARACTER_CODE` uses ALL_CAPS intentionally to signal a global configuration flag, not a regular parameter.

**Key change from v3:** Replaces `sklearn.Pipeline` with plain functions. The v3 plan had a critical bug: sklearn Pipeline requires intermediate steps to have `transform()` and the final step to have `predict()`, but `ChunkedRetrievalPredictor` (intermediate when rollup is enabled) lacked `transform()` and `CategoryRollUpTransformer` (final) lacked `predict()`, crashing 30 of 31 evaluation configs. The v4/v5 design eliminates this entire class of bug by using a simple function chain with a single `run_pipeline()` function — no fit/predict split, no redundant file I/O, no sklearn protocol constraints.
...
- `src/retriever.py:67-87` — `TfidfRetriever.search(query, top_k=5)` returns `list[SearchResult]` where each has `.code` (str), `.score` (float), `.description` (str), `.rank` (int). Scores are rounded to 4 decimal places.
...
No pipeline files exist yet (`src/pipeline_v5.py`, `src/pipeline_v4.py`, `src/pipeline_v3.py`, `src/pipeline.py`, etc. are all absent). This is a new file.

## Desired End State

A single new file `src/pipeline_v5.py` that:
...
- Contains a `__main__` block that builds `TfidfRetriever` once, then runs a single baseline + parameter sweep (27 configs) reusing the same retriever
- Runs successfully via `uv run python src/pipeline_v5.py` from project root

**Success Criteria:**
- [x] `src/pipeline_v5.py` exists and passes `uv run ruff check src/pipeline_v5.py`
- [x] `src/pipeline_v5.py` passes `uv run ruff format --check src/pipeline_v5.py`
- [x] `uv run python src/pipeline_v5.py` runs end-to-end without errors, exit code 0
- [x] Output contains exactly 1 metric row in the "Baseline" section (with `P=`, `R=`, `F1=`)
- [x] Output contains exactly 27 metric rows in the "Parameter Sweep" section
- [x] Output contains a `Best:` line with a dict including an `f1` key
...
- NOT importing numpy directly in `pipeline_v5.py` (it remains an indirect dependency via `TfidfRetriever` which uses scikit-learn and numpy internally)
...
|------|--------|-------|---------|
| `src/pipeline_v5.py` | CREATE | 1 | Complete multi-label retrieval pipeline with evaluation |
...
```mermaid
graph TD
    P1["Phase 1: Create pipeline_v5.py"]
...
`IGNORE_THREE_CHARACTER_CODE=True`. In the `__main__` block, `TfidfRetriever`
is built **once** and passed directly into `PipelineConfig` for both the
baseline and all 27 sweep configs.
...
flowchart TD
    ...
    KB --> Retriever["TfidfRetriever(kb)\n  built once in __main__\n  shared by baseline + all sweep configs\n  .search(query, top_k) → list[SearchResult]"]
...
| TF-IDF index sharing in sweep | Rebuild per config (`build_pipeline()` ×27) vs build once, share | Build once, share via `PipelineConfig` directly | All 27 sweep configs use the same KB with no `expand_kb`; rebuilding is wasteful. Building once and passing the retriever into `PipelineConfig` reduces index builds from 28 to 1. |
| `NotImplementedType` import style | Top-level `from types import NotImplementedType` vs `TYPE_CHECKING` guard | `TYPE_CHECKING` guard | With `from __future__ import annotations`, the import is never needed at runtime. Moving it under `TYPE_CHECKING` is best practice for typing-only imports. |
...
## Phase 1: Create pipeline_v5.py

### Overview
Create the complete `src/pipeline_v5.py` file containing all pipeline components as plain functions, evaluation helpers, and the main evaluation script.
...
- [x] File exists: `test -f src/pipeline_v5.py`
- [x] Linting passes: `uv run ruff check src/pipeline_v5.py`
- [x] Formatting passes: `uv run ruff format --check src/pipeline_v5.py`
  - If the format check fails, auto-fix with `uv run ruff format src/pipeline_v5.py` then re-run the check.
...
- `src/pipeline_v5.py` — the file created in Phase 1
...
**Expected runtime:** 3–8 minutes. The TF-IDF index is built exactly once at startup over 74,719 entries; all 28 evaluation runs (1 baseline + 27 sweep) share it. Progress is visible as metric rows print one at a time.

**Expected output structure:**
1. Header: `Multi-Label ICD-10 Retrieval - v5 Evaluation`
2. `Building TF-IDF index...` (single line, printed once)
3. Baseline section: `--- Baseline ---` followed by exactly 1 metric row containing `P=`, `R=`, `F1=`
4. Sweep section: `--- Parameter Sweep ---` followed by exactly 27 metric rows
5. Final line: `Best: {...}` with a dict containing keys `window`, `top_k`, `threshold`, `precision`, `recall`, `f1`

**If the script fails**, the most likely causes are:
1. **Import error for `knowledge_base` or `retriever`**: The `uv run` command should add `src/` to `sys.path` via `pyproject.toml` `pythonpath = ["src"]`. If not, add these two lines at the top of `src/pipeline_v5.py` (after `from pathlib import Path`, before `from knowledge_base import ...`):
    ```python
    import sys
...
#### Automated Verification:
- [x] Script runs without errors: `uv run python src/pipeline_v5.py` exits with code 0
- [x] Output contains "Best:" line with a configuration dict:
  ```bash
  uv run python src/pipeline_v5.py 2>&1 | grep "^Best:"
  ```
- [x] Output contains exactly 28 metric rows (1 baseline + 27 sweep):
  ```bash
  uv run python src/pipeline_v5.py 2>&1 | grep -c "P=.*R=.*F1="
  # Expected output: 28
  ```
...
### Integration Tests:
The `__main__` block of `pipeline_v5.py` runs the full pipeline over 250 CodiEsp dev documents and reports macro-averaged P/R/F1. This serves as the end-to-end verification.

### Manual Testing Steps:
1. Run `uv run python src/pipeline_v5.py` from project root
...
- v4 plan (superseded): `plan/future-plans/2026-02-26-multi-label-icd10-pipeline-v4.md`
- v4 review: `plan/future-plans/2026-02-26-multi-label-icd10-pipeline-v4.REVIEW.md`
