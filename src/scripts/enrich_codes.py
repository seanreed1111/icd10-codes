...
def main():
    ...
    # Reorder columns
    enriched = enriched[["ICD10-CM-CODE", "description", "category_code",
                          "category_description", "section", "chapter"]]

    PROCESSED.mkdir(parents=True, exist_ok=True)
    enriched.to_csv(PROCESSED / "icd10cm-codes-enriched-April-1-2026.csv", index=False)
    print(f"Wrote {len(enriched):,} rows to {PROCESSED / 'icd10cm-codes-enriched-April-1-2026.csv'}")
